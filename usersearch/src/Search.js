...
function Search() {
  ...
  function addHTML(res) {
    ...
    if (res.data.name) html += `<div>Name: ${res.data.name}</div>`;
    if (res.data.bio) html += `<div>Bio: ${res.data.bio}</div>`;
    if (res.data.location) html += `<div>Location: ${res.data.location}</div>`;
    if (res.data.public_repos)
      html += `<div>Public Repos: ${res.data.public_repos}</div>`;
    if (res.data.followers) {
      if (res.data.followers === 0) html += `<div>`;
      else html += `<div>Followers: ${res.data.followers}`;
  ...
  function handleSearch(e) {
    ...
    if (user === "" || user === null) {
      errorText.innerHTML = `
            <div style="color:red;">Provide a username.</div>`;
      userInfo.innerHTML = "";
    } else if (login !== user) {
